...
// import { MemoService } from "../../ememo/memo.service";
...
export class EmployeeViewComponent implements OnInit {
    ...
    if (this.deptControl !== null) {
      this.deptControl.valueChanges
        .pipe(
          ...
          switchMap(value => this.dataServices.getDepartment(value)
  ...
  private getDepartmentList(pageNumber) {
    ...
    this.dataServices.get_empTodeptMapping(this.empId, pageNumber)
      .subscribe((results: any[]) => {
        let datas = results["data"];
  ...
  getemployeeView(employee) {
    ...
    let employeename=this.sharedService.empname.value
    ...
    this.dataServices.getSingleEmployee(this.empId)
      .subscribe((results: any[]) => {
        let datas = results;
  ...
  SubmitClick() {
    ...
    if (this.chipSelectedDeptid.length !== 0) {
      ...
      this.dataServices.adddept_Toemployee(memojson, this.empId, this.Admin, this.Memo)
        .subscribe(res => {
          // console.log("res",res)
          let code = res.code
    ...
    if (this.chipRemovedDeptid.length !== 0) {
      ...
      memojson["department_id"] = JSON.parse(x)
      this.finaljson = JSON.stringify(Object.assign({}, memojson));
      this.dataServices.removedept_Fromemployee(memojson, this.empId)
        .subscribe(res => {
  ...
  activeInactive(employee, typeOfStatus) {
    ...
    this.dataServices.SingleEmployeeActiveInactive(dataToSubmit)
      .subscribe((results => {
        if (results.message == "Successfully Updated") {
          this.onCancel.emit()
          if (dataToSubmit.status == 1) {
            this.notification.showSuccess("Successfully Activated")
          }
          if (dataToSubmit.status == 0) {
