...
export class ProductmasterEditComponent implements OnInit {
  ...
  //         }),
  //         switchMap(value => this.masterservice.getapsubcat_LoadMore(value, 1)
  //           .pipe(
  //             finalize(() => {
  //               this.isLoading = false
  //             }),
  ...
  //   this.masterservice.productmasterEditForm(this.productid, this.productCreateForm.value)
  //   .subscribe(result => {
...
constructor(private shareservice:ShareService,private formbuilder: FormBuilder, private masterservice: Master2Service,
  ...
  console.log(this.productForm.get('product_isrcm').value?"Y":"N");
    console.log(this.productForm.get('product_isblocked').value?"Y":"N");
      ...
      if(this.productForm.get('name').value==undefined || this.productForm.get('name').value=='' || this.productForm.get('name').value==null || this.productForm.get('name').value==""){
          ...
          this.disableSubmit = true;
          this.spinner.show();
          this.masterservice.productmasterEditForm(this.productid, data)
          .subscribe(result => {
            this.spinner.hide();
            if (result.code === "INVALID_DATA" && result.description === "Invalid Data or DB Constraint") {
              this.notification.showWarning("Duplicate!Code ...")
