...
import {ShareService} from 'src/app/Master/share.service'
...
@Component({
  selector: 'app-emp-details-edit',
  templateUrl: './emp-details-edit.component.html',
  styleUrls: ['./emp-details-edit.component.scss'],
  providers:[{provide:DateAdapter,useClass:PickDateAdapter},
  {provide:MAT_DATE_FORMATS,useValue:PICK_FORMATS}]
})
export class EmpDetailsEditComponent implements OnInit {
    ...
    has_ccpage:number=1;
    Gender:any={1:'Male',2:"Female",3:"TransGender",0:'Others'};
    ...
    constructor(private router:Router,private shareservice:ShareService,private datepipe:DatePipe,private spinner:NgxSpinnerService,private notification:NotificationService,private fb:FormBuilder,private masterservice:masterService) { }
    ...
    ngOnInit(): void {
      ...
      pipe(
        ...
        switchMap((value:any)=>this.masterservice.getCityDropDownscroll(value,1).pipe(
          finalize(()=>{
            this.isLoading=false;
          })
        )
      ...
      let data:any=this.shareservice.empeditid.value;
      this.getdata(data);
  
    }
    getdata(id:any){
      this.masterservice.getEmployeeedit(id).subscribe(data=>{
        this.createemployss.get('code').patchValue(data.code);
        this.createemployss.get('name').patchValue(data.name);
        this.createemployss.get('dob').patchValue(data.dob);
        this.createemployss.get('doj').patchValue(data.doj);
        this.createemployss.get('gender').patchValue(this.Gender[data['gender']]);
        this.createemployss.get('designation').patchValue(data.designation);
        this.createemployss.get('doj').patchValue(data.doj);
        if(data.phone_no==undefined || data.phone_no==null){
          this.createemployss.get('mobilenumber').patchValue('');
        }
        else{
          this.createemployss.get('mobilenumber').patchValue(data.phone_no);
        }
        this.createemployss.get('hierarchy').patchValue(data.hierarchy);
        this.createemployss.get('branch').patchValue(data.branch);
        this.createemployss.get('bsname').patchValue(data.bs);
        this.createemployss.get('ccname').patchValue(data.cc);
        this.createemployss.get('line1').patchValue(data['address'].line1);
        this.createemployss.get('line2').patchValue(data['address'].line2);
        this.createemployss.get('line3').patchValue(data['address'].line3);
        this.createemployss.get('pincode').patchValue({'id':data['address'].pincode_id,'no':data['address'].pincode_no});
        this.createemployss.get('city').patchValue({'id':data['address'].city_id,'name':data['address'].city_name});
        this.createemployss.get('district').patchValue({'id':data['address'].district_id,'name':data['address'].district_name});
        this.createemployss.get('state').patchValue({'id':data['address'].state_id,'name':data['address'].state_name});
        this.createemployss.get('contacttype').patchValue(data['contact'].type_id);
        // this.createemployss.get('personname').patchValue(data['address'].state);
        this.createemployss.get('landline1').patchValue(data['contact'].landline);
        this.createemployss.get('landline2').patchValue(data['contact'].landline2);
        this.createemployss.get('contactnumber').patchValue(data['contact'].mobile);
        this.createemployss.get('contactnumber').patchValue(data['contact'].mobile2);
        this.createemployss.get('conemailid').patchValue(data['contact'].email);
      });
    }
    ...
    getccdata(){
      ...
      this.masterservice.getccdatafilter(this.createemployss.get('bsname').value.id,this.createemployss.get('bsname').value,1).subscribe(data=>{
    ...
    employeecreatedata(){
      ...
      let data:any={
        "id":this.shareservice.empeditid.value,
